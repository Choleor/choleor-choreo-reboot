...
class Initializer:
    ...
    @staticmethod
    def choreo_csv_extractor(file="/home/jihee/choleor_media/choreo/choreo_data.csv"):
        Initializer.choreo_csvinfo_dict = Initializer.csv_reader.read(file,
                                                                      **{'col1': 'choreo_id', 'col2': 'choreo_slice_id',
        ...
        Initializer.data_n = 312
        # Initializer.data_n = len(Initializer.choreo_csvinfo_dict['choreo_slice_id'])

    @staticmethod
    def meta_csv_extractor(file="/home/jihee/choleor_media/output.csv"):
        Initializer.choreo_csvinfo_dict['bpm'] = [0 if i is "" else float(i) for i in
    ...
    def __set_parameters__():
        ...
        For DB insertion
        """
        for i in range(Initializer.data_n):
    ...
    @staticmethod
    def download_video(vid_links, cslice_ids, start, fps, start_frame, end_frame, bpm):
        """
        vidlink = [
        ...
        for idx in range(312):  ## 나중에 len(vidlinks로 바꾸기
            with ydl:
    ...
    def insert_db():
        for i in range(Initializer.data_n):
            ...
            if i != 0 and chor_id == Initializer.choreo_csvinfo_dict['choreo_id'][i - 1]:
                continue
            c_obj = Choreo(choreo_id=Initializer.choreo_csvinfo_dict['choreo_id'][i],
                           download_url="http://www.youtube.com/watch?v=" +
                                        Initializer.choreo_csvinfo_dict['choreo_id'][i],
                           bpm=Initializer.choreo_csvinfo_dict['bpm'][i])
            Choreo.save(c_obj)

        for k in range(Initializer.data_n):
            ...
            try:
                ...
                # Initializer.choreo_csvinfo_dict['movement']로 빈 배열 나중에 바꿀 것
            except:
                ChoreoSlice.objects.filter(
                    choreo_slice_id=Initializer.choreo_csvinfo_dict['choreo_slice_id'][k]).delete()
                pass
    ...
    def set_thumbnail():
        # input_file = []
        # cids = Initializer.choreo_csvinfo_dict['choreo_id']
        # cslice_ids = Initializer.choreo_csvinfo_dict['choreo_slice_id']
        #
        # for idx, csid in enumerate(cslice_ids):
        #     cids += [csid.split("~")[0]]
        #     input_file.append(f'/home/jihee/choleor_media/choreo/SLICE/{cids[idx]}/{cslice_ids[idx]}.mp4')

        # 24fps로 변환
        for idx in range(0, Initializer.data_n):
            print(Initializer.choreo_csvinfo_dict['choreo_id'][idx])
            print(Initializer.choreo_csvinfo_dict['choreo_slice_id'][idx])
            vidcap = cv2.VideoCapture(
                '/home/jihee/choleor_media/choreo/SLICE/{}/{}.mp4'.format(Initializer.choreo_csvinfo_dict['choreo_id'][idx],
    ...
    def initialize():
        ...
        print(Initializer.choreo_csvinfo_dict['choreo_slice_id'])
        print(Initializer.choreo_csvinfo_dict['start_t'])
        print(Initializer.choreo_csvinfo_dict['bpm'])
        # only execute when downloading video
        # Initializer.download_video(Initializer.choreo_csvinfo_dict['url'],
